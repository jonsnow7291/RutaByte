...
def listar_sedes(db: Session = Depends(get_db)) -> list[Sede]:
    stmt = select(Sede).order_by(Sede.activa.desc(), Sede.nombre.asc())
    return list(db.scalars(stmt).all())


@router.delete("/{sede_id}")
def toggle_sede(sede_id: int, db: Session = Depends(get_db)) -> dict[str, str | int]:
    sede = db.get(Sede, sede_id)
    if sede is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Sede no encontrada")
    ...
    db.commit()
    db.refresh(sede)
    accion = "activada" if sede.activa else "desactivada"
    return {"message": f"Sede {accion} correctamente", "id": sede.id}
